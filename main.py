    ...
    with open('user_ids.txt', 'r', encoding='utf-8') as file:
        ...
        if not string in user_ids_list:
            if re.match(r"[^@]+@[^@]+\.[^@]+", user_text):
                ...
                txt = 'Стандартна ціна: 200PLN \nЦіна зараз - 150PLN \nНе впустіть шанс скористатися передовою технологією за вигідною ціною!'
                txt2 = 'Найближчим часом(від 30хв.) ви отримаєте повідомлення про підтвердження оплати та доступ до боту.'
                txt3 = 'Через крадіжку сайтів ми не можемо надати безкоштовний доступ до боту, сподіваємось на розуміння.'
                txt4 = 'Уважно! Вказані емайли в чаті та на сторінці з оплатою повинні співпадати, якщо вони будуть різні то оплата не буде підтверджена.'
                txt5 = 'В боті є близько 80 сайтів, в майбутньому цей список буде поповнятись.'
                txt6 = 'Після оплати ви назавжди отримуєте доступ до боту, повернення коштів тимчасово недоступне.'
                text = f'{txt}\n{txt2}\n{txt3}\n{txt4}\n{txt5}\n{txt6}'
                await bot.send_message(
                    message.chat.id,
                    text=text,
